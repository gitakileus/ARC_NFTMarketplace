...
import { useGetAirdropRewardQuery } from 'services/reward';
...
export default function RewardAirdropRewards() {
  ...
  const { data: airdropRewardResponse } = useGetAirdropRewardQuery(account);
  ...
  useEffect(() => {
    if (airdropRewardResponse && airdropRewardResponse.data) {
      setAirdropRewards(airdropRewardResponse.data.rewards);
    }
  }, [airdropRewardResponse]);
  ...
  return (
    <>
      ...
      >
        ...
        <span>{airdropRewards ? airdropRewards.toFixed(2) : '0.00'} ARC</span>
      </div>
      <Button
        sx={{ width: '100%', mt: '180px', fontSize: '16px', pt: '16px', pb: '16px' }}
        size="large"
        variant="contained"
        href=""
        target="_blank"
